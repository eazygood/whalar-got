...
function getRelationships(relationshipsId: number, characterId: number, relations: string, type: RelationTypes): RelationshipsDb {
    return {
        id: relationshipsId,
...
function getAction(actionId: number, characterId: number, actionTo: string, type: ActionTypes): ActionsDb {
    return {
        id: actionId,
        character_id: characterId,
        action_to: actionTo,
...
function getActor(actorId: number, characterId: number, actorName: string, actorLink: string | null): ActorsDb {
    return {
        id: actorId,
        character_id: characterId,
        name: actorName,
...
function prepareDataForDb(data: Characters[]): any {
    ...
    for (const character of data) {
        ...
        character.siblings?.forEach(relation => {
            const relationshipsDb = getRelationships(relationshipId, characterId, relation, RelationTypes.sibling);
            relationships.push(relationshipsDb);
            relationshipId += 1;
        });

        character.marriedEngaged?.forEach(relation => {
            const relationshipsDb = getRelationships(relationshipId, characterId, relation, RelationTypes.marriedEngaged);
            relationships.push(relationshipsDb);
            relationshipId += 1;
        });
        ...
        if (character.actorName) {
            const actorDb = getActor(actorId, characterId, character.actorName, character.actorLink ?? null);
            actors.push(actorDb);
            actorId += 1;
        }
...
function nameToIdFroRelationships(relationships: RelationshipsDb[], nameToId: { [key: string]: number }): RelationshipsDb[] {
...
function nameToIdFroActions(actions: ActionsDb[], nameToId: { [key: string]: number }): ActionsDb[] {
    actions.forEach(a => {
