...
export async function findMany({ app, searchQuery }: { app: FastifyInstance, searchQuery?: SearchCharactersQuerystring }) {
    const conn = app.knex;
    return await db.buildAndRun<Character[]>({ 
        app, 
        callback: async (conn) => {
...
export async function createOne({ app, data, transaction}: { app: FastifyInstance, data: CharacterToAdd, transaction?: Knex.Transaction }) {
    return await db.buildAndRun<Character>({ 
        app, 
        transaction,
        callback: async (conn) => {
...
export async function updateOne({ app, id, data, transaction}: { app: FastifyInstance, id: number; data: Character, transaction?: Knex.Transaction }) {
    return await db.buildAndRun<Character>({ 
        app, 
        transaction,
        callback: async (conn) => {
...
export async function deleteOne({ app, id, transaction}: { app: FastifyInstance, id: number; transaction?: Knex.Transaction }): Promise<number> {
